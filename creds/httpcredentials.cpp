...
void HttpCredentials::fetch(Account *account)
{
    ...
    //QSettings *settings = account->settingsWithGroup(/*Theme::instance()->appName()*/ "test");
    QSettings *settings;
    ...
    if (_ready) {
        ...
        ReadPasswordJob *job = new ReadPasswordJob(/*Theme::instance()->appName()*/ "test");
...
QString HttpCredentials::queryPassword(bool *ok)
{
    if (ok) {
        QString str = QInputDialog::getText(0, tr("Enter Password"),
                                     ...
                                     .arg(/*Theme::instance()->appName()*/ "test", _user),
...
void HttpCredentials::invalidateToken(Account *account)
{
    ...
    //TODO: gili
    //DeletePasswordJob *job = new DeletePasswordJob(/*Theme::instance()->appName()*/ "test");
    //QSettings *settings = account->settingsWithGroup(/*Theme::instance()->appName()*/ "test");
    //settings->setParent(job); // make the job parent to make setting deleted properly
    //job->setSettings(settings);
...
void HttpCredentials::persist(Account *account)
{
    ...
    WritePasswordJob *job = new WritePasswordJob(/*Theme::instance()->appName()*/ "test");
    //QSettings *settings = account->settingsWithGroup(/*Theme::instance()->appName()*/ "test");
    //settings->setParent(job); // make the job parent to make setting deleted properly
    //job->setSettings(settings);
