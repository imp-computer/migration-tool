...
QNetworkReply *AbstractNetworkJob::putRequest(const QUrl &url, QIODevice *data)
{
    return addTimer(_account->putRequest(url, data));
}

QNetworkReply *AbstractNetworkJob::putRequest(const QString &relPath, QIODevice *data)
{
    return addTimer(_account->putRequest(relPath, data));
}
...
void AbstractNetworkJob::slotFinished()
{
    ...
    _responseTimestamp = QString::fromLatin1(_reply->rawHeader("Date"));
...
{
    emit finished(reply()->error(), reply()->url());
    return true;
}
...
UploadFileJob::UploadFileJob(Account *account, const QString &sourceFilePath, const QString &remotePath, QObject *parent)
    : AbstractNetworkJob(account, remotePath, parent)
...
{
    qDebug() << "UploadFileJob opening file: " << _filePath;
    QFile *file = new QFile(_filePath);
    if (!file->open(QIODevice::ReadOnly)) {
        qDebug() << "File read error " + _filePath +" Code: "
                    << file->error();
        return;
    }
    setReply(putRequest(path(), file));
    setupConnections(reply());
    AbstractNetworkJob::start();
}

bool UploadFileJob::finished()
{
    qDebug() << "UploadFileJob";
    qDebug() << reply()->error();
