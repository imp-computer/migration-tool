...
protected:
    ...
    QNetworkReply* putRequest(const QUrl &url, QIODevice *data);
    QNetworkReply* putRequest(const QString &relPath, QIODevice *data);
...
signals:
    void finished(QNetworkReply::NetworkError, QUrl url);
...
/**
 * @brief The UploadFileJob class
 */
class UploadFileJob : public AbstractNetworkJob {
    Q_OBJECT
public:
    explicit UploadFileJob(Account *account, const QString &sourcePath, const QString &remotePath, QObject *parent = 0);
    void start();

signals:
    void fileUploaded(qint64 totalBytes, qint64 availableBytes);
