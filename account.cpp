...
void Account::save()
{
    QScopedPointer<QSettings> settings(settingsWithGroup(Theme::instance()->appName() "test"));
...
QNetworkReply *Account::putRequest(const QString &relPath, QIODevice *data)
{
    QNetworkRequest request(concatUrlPath(url(), relPath));
    return _am->put(request, data);
}

QNetworkReply *Account::putRequest(const QUrl &url, QIODevice *data)
{
    QNetworkRequest request(url);
    return _am->put(request, data);
}
